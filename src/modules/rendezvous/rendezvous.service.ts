...
import { RendezvousStatus } from 'src/common/enums/rendezvous-status.enum';
...
private readonly disponibiliteRepo: Repository<Disponibilite>,
  ...
  async createRendezvous(dto: CreateRendezvousDto): Promise<Rendezvous> {
    ...
    await this.checkDisponibiliteAndConflict(dto.date, dto.heure, dto.veterinaireId);
  ...
  async addAnimalToRendezvous(rendezvousId: number, animalId: number) {
    const rendezvous = await this.rendezvousRepo.findOne({ where: { id: rendezvousId } });
    if (!rendezvous) {
      throw new NotFoundException(`Rendez-vous with id ${rendezvousId} not found`);
    }
...
async checkDisponibiliteAndConflict(
  ...
  const isWithinDisponibilite = disponibilites.some((d) => {
    ...
    const isSameDay = !d.estExceptionnelle || moment(d.dateExceptionnelle).isSame(date, 'day');
  ...
  if (!isWithinDisponibilite) {
    throw new ConflictException('Le vétérinaire n\'est pas disponible pendant toute la durée du rendez-vous.');
  }
  ...
  if (hasOverlap) {
    throw new ConflictException('Un autre rendez-vous est déjà prévu pendant cette période.');
  }
}
