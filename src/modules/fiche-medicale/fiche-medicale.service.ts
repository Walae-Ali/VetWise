...
export class FicheMedicaleService {
  /*constructor(
   ...
   async create(dto: CreateFicheMedicaleDto) {
    const dossier = await this.dossierRepo.findOneByOrFail({
      id: dto.dossierId,
    });
    const veterinaire = await this.vetRepo.findOneByOrFail({
  ...
  findOne(id: number) {
    return this.ficheRepo.findOne({
      where: { id },
      relations: ['dossier', 'veterinaire'],
    });
  }
